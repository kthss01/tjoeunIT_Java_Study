...
class InnerEx8 {
	public static void main(String[] args) {
		...
		// WindowListener는 인터페이스라 모두 구현해야함
//		f.addWindowListener(new WindowListener() {
//			public void windowClosing(WindowEvent e) {
//				e.getWindow().setVisible(false); // 비디오 메모리에서 날려버리는 거
//				e.getWindow().dispose(); // 메인 메모리에서 날려버리는 거
//				System.exit(0);
//			}
//
...
//		); // 리스너의 끝
		...
		// 위에 것을 클래스 따로 만들어서 해본거
		EventProcClass eventClass = new EventProcClass();
		b.addActionListener(eventClass);

	} // main메서드의 끝
} // InnerEx8클래스의 끝

class EventProcClass implements ActionListener {
